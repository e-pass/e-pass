...
def _get_section_object_from_db(section_id: int, need_return=False) -> Type['SectionModel'] | None:
...
def _get_pass_object_from_db(pass_id: int) -> Type['PassModel'] | None:
    """Функция проверки наличия записи в БД."""
...
def _check_expiration_total(valid_from: datetime, valid_until: datetime) -> None:
    """Функция полной проверки введённого периода действия и сравнения с текущей датой"""
    if not all((_check_expiration_period(valid_from, valid_until), _check_expiration_date(valid_until))):
        raise serializers.ValidationError({'message': 'Введён некорректный период действия.'})


def _check_periods_crossing(exist_range: Type['Range'], new_range: Type['Range']) -> int | None:
...
def _check_existing_passes(validated_data: dict) -> None:
    ...
    for st_pass in student_passes:
        ...
        if _check_periods_crossing(exist_range, new_range):
            raise serializers.ValidationError(
                {'message': 'На эти даты уже имеется абонемент для данного пользователя.'})


def _check_last_entry(last_entry_time: datetime) -> None:
...
def _check_unused_lessons_quantity(pass_obj: Type['PassModel'], quantity_entries: int) -> None:
...
def _check_pass_activity(pass_obj: Type['PassModel']) -> None:
...
def _check_pass_is_paid(pass_obj: Type['PassModel']) -> None:
    if not pass_obj.is_paid:
        raise serializers.ValidationError('Абонемент не оплачен.')


def _check_pass_valid_from(pass_obj: Type['PassModel']) -> None:
    if datetime.date.today() < pass_obj.valid_from:
        raise serializers.ValidationError(
            f'Ошибка. Абонемент будет действителен для сканирования с {pass_obj.valid_from}')


def _check_pass_before_check_in(pass_id: int) -> Type['PassModel']:
    """Функция получения абонемента из БД и проверки возможности сканирования"""
    from membership_passes.models import EntryModel
    pass_obj = _get_pass_object_from_db(pass_id=pass_id)
    entries = EntryModel.objects.filter(to_pass=pass_id).order_by('created_at')
    quantity_entries = entries.count()

    _check_pass_activity(pass_obj=pass_obj)
    _check_pass_is_paid(pass_obj=pass_obj)
    _check_pass_valid_from(pass_obj=pass_obj)
    if quantity_entries > 0:
        if not pass_obj.is_unlimited:
            _check_unused_lessons_quantity(pass_obj=pass_obj, quantity_entries=quantity_entries)
        _check_last_entry(last_entry_time=entries.last().created_at)
